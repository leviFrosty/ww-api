> Mirror of `docs/buddies-protocol.md` in the witness-work app repo, which holds the canonical copy; change that one first.
...
The remover calls `slot/remove` (their inbox, the other person's slot) and `slot/leave` (the other person's inbox, their own slot), then purges locally. The other side sees the slot vanish from `inbox/sync` and purges too. No push.
